...
struct SendTcpMessageIntent: AppIntent {
    ...
    @Parameter(title: "Message", description: "Message to send over TCP")
    var message: String?
    ...
    func perform() async throws -> some ReturnsValue<String> {
        do {
            let resolved = try await IntentResolutionHelper.resolveRequest(
                ...
                typeFilter: .tcp,
                disambiguate: {
                    try await $request.requestDisambiguation(among: $0, dialog: IntentDialog("Choose a TCP request"))
                }
            )
            ...
            let response = try await IntentExecutionService.executeTcp(
