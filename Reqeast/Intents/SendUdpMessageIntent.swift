...
struct SendUdpMessageIntent: AppIntent {
    ...
    @Parameter(title: "Message", description: "Message to send as UDP datagram")
    var message: String?
    ...
    func perform() async throws -> some ReturnsValue<String> {
        do {
            let resolved = try await IntentResolutionHelper.resolveRequest(
                ...
                typeFilter: .udp,
                disambiguate: {
                    try await $request.requestDisambiguation(among: $0, dialog: IntentDialog("Choose a UDP request"))
                }
            )
            ...
            let response = try await IntentExecutionService.executeUdp(
